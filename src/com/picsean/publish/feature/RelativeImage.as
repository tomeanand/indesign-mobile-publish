		...
		private function getSubFeatures():void{
			...
			for (var i:int=0; i<subFeat.length; i++){
				...
				if(item.name == WCONTENT){
					...
					///// Relative image feature supports Draw and video feature.
					
					if(str.indexOf(Configuration.TYPE_VIDEO_FEATURE) != -1){
						feature = new VideoFeature(item as Group);
		...
		public function badHide(isHide:Boolean,ftye:String):void{
			...
			for (var i:int=0; i<subFeat.length; i++){
				...
				if(item.name == WCONTENT){
					...
					var str:String = item.name;
					if(str.indexOf(Configuration.TYPE_VIDEO_FEATURE) < 0){
